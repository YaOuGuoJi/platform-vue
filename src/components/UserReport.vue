<template>
  <div id="all">
    ...
    <template v-if="userReportData">
      ...
      <div class="survey">
        <p id="summary">
          {{year}}年{{showMonth == '' ? showMonth : showMonth + '月'}}您在亚欧国际小镇共下单{{userReportData.orderNumber}}次,共消费{{userReportData.totalPrice}}元，超过了{{userReportData.beyondPercent}}%的人，
          排名第{{rank}}
        </p>
      </div>
      ...
      <div class="max_price_order">
        <p id="maxPrice">
          {{year}}年{{showMonth == '' ? showMonth : showMonth + '月'}}消费最高订单：{{userReportData.maxPriceOrder.day}}您在{{userReportData.maxPriceOrder.shopName}}购买了{{userReportData.maxPriceOrder.productList}},
